...
class TwitterProxy {
	 ...
	 */
	public function __construct($oauth_access_token, $oauth_access_token_secret, $consumer_key, $consumer_secret, $search, $type, $last_id, $count = 100) {

		$this->config = array_merge($this->config, compact('oauth_access_token', 'oauth_access_token_secret', 'consumer_key', 'consumer_secret', 'search', 'type', 'last_id', 'count'));

		$this->whitelist['search/tweets.json?q=' . $this->config['search'] . '&result_type='  . $this->config['type'] . '&max_id='  . $this->config['last_id'] . '&count='  . $this->config['count']] = true;
	}
