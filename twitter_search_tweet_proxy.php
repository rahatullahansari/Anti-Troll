...
class TwitterProxy {
	 ...
	 */
	public function __construct($oauth_access_token, $oauth_access_token_secret, $consumer_key, $consumer_secret, $search, $count = 1000) {

		$this->config = array_merge($this->config, compact('oauth_access_token', 'oauth_access_token_secret', 'consumer_key', 'consumer_secret', 'search', 'count'));

		$this->whitelist['statuses/user_timeline.json?screen_name=' . $this->config['search'] . '&count=' . $this->config['count']] = true;
	}
